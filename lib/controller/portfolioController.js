...
const getHoldings = async function (portfolioId) {
  ...
  let holdings = PortfolioHelper.getMetaHoldingsObject(trades)

  holdings = PortfolioHelper.normalizeMetaHoldingsObject(holdings)

  if (_.isEmpty(holdings)) {

    return 'There are no holdings as of now.'
  }
